setwd("C:/Users/davin/Documents/PhD/Writing/Heterogeneity/Supp_Data/Tool_promoter_platecurver/210623_rep3_rep4/")
library(readxl)
...
ggplotify.bc<-function(p){ggplot(p, aes(Time,value, col=variable)) + 
    ...
    scale_y_continuous(limits = c(-0.1,1.2), breaks = seq(0, 1.2, by = 0.4))}
...
ggplotify.bcsm<-function(p){ggplot(p, aes(Time,value, col=variable)) + 
    ...
    mytheme +
    scale_x_continuous(limits = c(0, 28), breaks = seq(0, 28, by = 4)) +
    scale_y_continuous(limits = c(-0.1,0.6), breaks = seq(0, 0.6, by = 0.1))}
...
sortfiles<-mixedsort(list.files(pattern='210623_t'))
sortfiles


#tidy up function - REMEMBER TO CHANGE TIME POINTS
tidy.up<-function(newseries){
  Time<-c("0","1","2","4","5","6","7","8","9","10","11","11.5",
          "24","25","26","28","29","32","48","49","52","53","54")
  tns<-t(newseries)
